...
function applyLineFilters(pixels, width, height) {
    ...
    const threshold = 60; // Seuil élevé pour des lignes plus nettes et moins de bruit
...
// **دالة onmessage لتنفيذ عملية دمج البيانات (Fusion)**
self.onmessage = function(e) {
    const { imageData, width, height, scaleFactor } = e.data; 
    const originalPixels = imageData.data;
    
    // 1. توليد المخطط الهندسي النظيف
    const sketchPixels = applyLineFilters(originalPixels, width, height); 
    
    // 2. دمج البيانات: دمج الخطوط السوداء على الصورة الملونة الأصلية
    const finalSketchPixels = new Uint8ClampedArray(originalPixels.length);

    for (let i = 0; i < originalPixels.length; i += 4) {
        
        // إذا كان اللون في المخطط أسود، استخدم الأسود
        if (sketchPixels[i] === 0) {
            finalSketchPixels[i] = 0;      
            finalSketchPixels[i + 1] = 0;  
            finalSketchPixels[i + 2] = 0;  
            finalSketchPixels[i + 3] = 255; 
        } else {
            // إذا كان أبيض، استخدم ألوان الصورة الأصلية
            finalSketchPixels[i] = originalPixels[i];
            finalSketchPixels[i + 1] = originalPixels[i + 1];
            finalSketchPixels[i + 2] = originalPixels[i + 2];
            finalSketchPixels[i + 3] = originalPixels[i + 3]; 
        }
    }
    
    // 3. تحليل القياسات (باستخدام الخطوط النظيفة)
    const analysisResults = analyzeImage(sketchPixels, width, height, scaleFactor);
