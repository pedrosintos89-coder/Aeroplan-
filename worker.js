        ...
        for (let x = 1; x < width - 1; x++) {
            ...
            for (let ky = -1; ky <= 1; ky++) {
                for (let kx = -1; kx <= 1; kx++) {
                    ...
                    const idx = ((y + ky) * width + (x + kx)); // Fixed index calculation
...
function getBuildingBoundingBox(pixels, width, height) {
    ...
    for (let y = 0; y < height; y++) {
        if (y % 20 === 0) {
            self.postMessage({ type: 'progress', percent: Math.round((y / height) * 100 * 0.4) });
        }
...
function analyzeGroundImages(groundImageDataArray) {
    ...
    if (groundImageDataArray && groundImageDataArray.length > 0) {
        groundImageDataArray.forEach((data, index) => {
            // Reconstitute ImageData from buffer for analysis (Note: data.data is a buffer in worker)
            const pixels = new Uint8ClampedArray(data.data);
            const width = data.width;
            const height = data.height;

            // Simple Heuristic for Window Shape Detection
            const aspect = width / height;

            if (aspect < 0.8 || width < 150) { 
                 // Tall/Narrow or smaller images often imply ornamental/arched windows in classic style
                archedScore += 2;
            } else if (aspect > 1.2 || height < 150) { 
                // Wide/Short images suggest a more rectilinear, square style (or flat walls)
                squareScore += 1;
            } else {
                // If ground image looks like a square facade itself, favor it being square or arched
                if (index % 2 === 0) {
                    squareScore++;
                } else {
                    archedScore++;
            ...
            self.postMessage({ type: 'progress', percent: 40 + Math.round(((index + 1) / groundImageDataArray.length) * 100 * 0.4) });
        });
    }

    if (archedScore > squareScore * 1.5) { // Needs stronger evidence for arched
...
function analyzeImage(aerialPixels, width, height, scaleFactor, floorCount, windowType, groundImageDataArray) {
    ...
    // Heuristic for window count based on density
    const area = box.width * box.height;
    const density = blackPixelCount / area; 
    const windowFactor = 0.0003; // Adjusted density factor for better count
...
self.onmessage = function(e) {
    ...
    const aerialPixels = new Uint8ClampedArray(aerialImageData.data.buffer);
